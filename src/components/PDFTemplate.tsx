...
const PDFTemplate: React.FC<PDFTemplateProps> = ({
  ...
  return (
    <div className="bg-white mx-auto print:shadow-none">
      {/* Render Report Pages (Paginated) */}
      {(resultsChunks || [results]).map((pageResults, pageIndex) => (
        <div
          key={pageIndex}
          className="pdf-content-page bg-white mx-auto print:shadow-none"
          style={{
            width: '210mm',
            minHeight: '297mm',
            padding: '20mm 25mm',
        ...
        >
          ...
          {/* Metadata/Warnings - First Page Only */}
          {pageIndex === 0 && (
            <>
              {showAadtWarning && (
          ...
          <div className="mb-12">
            <h4 className="text-xs font-bold text-gray-700 uppercase tracking-wider mb-3">
              {pageIndex === 0 ? 'Calculated Results' : 'Calculated Results (Continued)'}
            </h4>
            {pageIndex === 0 && (
